...
void drive_left(int power, MotorMode motorMode);
void drive_right(int power, MotorMode motorMode);
...
void drive_left(int power, MotorMode motorMode)
{
	...
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, power + offset);
}

void drive_right(int power, int direction)
{
	int offset = 0;
	if (power != 0) offset = 250;
	else offset = 0;
	...
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, power + offset);
}
